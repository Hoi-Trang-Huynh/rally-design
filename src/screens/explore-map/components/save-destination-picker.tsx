...
type SaveDestinationPickerProps = {
  placeName: string;
  onClose: () => void;
  onSave: (destination: "personal" | "shared", notes: string, tags: string[], collectionId?: string | null, sessionIds?: string[]) => void;
  collections: SavedCollection[];
  sessions: RallySession[];
};

export default function SaveDestinationPicker({ placeName, onClose, onSave, collections, sessions }: SaveDestinationPickerProps) {
  ...
  const handleConfirm = () => {
    if (destination) onSave(destination, notes, [], destination === "personal" ? selectedCollection : null, destination === "shared" ? selectedSessionIds : undefined);
  };

  const stepTitle: Record<Step, string> = { pick: "Save to...", "pick-personal": "Personal Library", "pick-shared": "Select a trip to save to", details: "Add Details" };
  ...
  return (
    <div className="absolute inset-0 z-[60] flex flex-col justify-end" onClick={handleClose}>
      ...
      <div className={`relative z-10 flex max-h-[75vh] flex-col rounded-t-[24px] bg-white transition-transform duration-300 ease-out ${visible ? "translate-y-0" : "translate-y-full"}`}
        ...
        {step === "pick-personal" && (
          <div className="flex flex-col gap-[6px] overflow-y-auto px-[16px] py-[14px]">
            <button onClick={() => { setSelectedCollection(null); setStep("details"); }}
              className="flex items-center gap-[12px] rounded-[14px] bg-[#f9f9f8] px-[14px] py-[12px] text-left transition-all duration-150 active:scale-[0.98] active:bg-[#f0f0ef]">
            ...
            {collections.map((col) => (
              <button key={col.id} onClick={() => { setSelectedCollection(col.id); setStep("details"); }}
                className="flex items-center gap-[10px] rounded-[12px] bg-[#f9f9f8] px-[12px] py-[10px] text-left transition-all duration-150 active:scale-[0.98] active:bg-[#f0f0ef]">
                ...
                <FolderOpen size={14} className="shrink-0 text-[#b4b4b3]" />
              </button>
            ))}
            {!showCreateCollection ? (
              ...
              <div className="flex items-center gap-[8px] rounded-[12px] border-[1.5px] border-[#ff6733]/30 bg-[#ff6733]/5 px-[12px] py-[10px]">
                ...
                <button onClick={() => { if (newCollectionName.trim()) { setSelectedCollection(`new-${newCollectionName}`); setShowCreateCollection(false); setStep("details"); } }}
          ...
          <div className="flex max-h-[60vh] flex-col gap-[12px] overflow-y-auto px-[16px] py-[14px]">
            <div className="flex items-center gap-[6px] rounded-[10px] bg-[#f0f7ff] px-[10px] py-[6px]">
              ...
              <p className="flex-1 text-[11px] font-semibold text-[#3b82f6]">
                {destination === "personal" ? (selectedCollection ? `Saving to "${collections.find((c) => c.id === selectedCollection)?.name ?? newCollectionName}"` : "Saving to All Saved") : `Saving to ${selectedSessionIds.length} trip${selectedSessionIds.length > 1 ? "s" : ""}`}
              </p>
